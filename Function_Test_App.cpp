...
/*
 *内联函数：inline
 *特性：编译器会自动拒绝内联很长很复杂的函数，当类内或者手动给内敛的函数太复杂，编译器就不会使用内联
 *作用：会在函数调用处直接展开函数，而不是跳转到函数，会导致代码体积变大，但是可以省跳转时间
 *适用：短小、需要频繁调用的代码，可以省去函数调用时间开销，但是代码长、复杂、递归的函数会导致体积巨大
 *使用：必须把定义和声明放在一起，都写在头文件，普通函数的声明写在头文件，定义写在cpp
 *      inline int add(int a, int b)
 *      {
 *          return a + b;
 *      }
 *隐式inline：写在类定义里实现的成员函数自动是内联，只有声明在类内，定义在内外的才需要手动加inline
 */
